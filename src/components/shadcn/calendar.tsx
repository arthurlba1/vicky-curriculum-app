  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        button_previous: cn(
          buttonVariants({ variant: variant === 'text' ? 'link' : buttonVariant }),
...
function CalendarForm({ name, variant = 'default', placeholder = 'Pick a date', ...props }: CalendarFormProps) {
            ...
            <Popover>
                <PopoverTrigger asChild>
                  <FormControl>
                    ...
                    >
                      {field.value ? (
                        <span className={`${variant === 'text' ? 'text-sm text-foreground' : 'text-muted-foreground'}`}>{format(field.value, "PPP")}</span>
                      ) : (
                        <span className="text-muted-foreground/50 text-sm">{placeholder}</span>
                      )}
                      {!props.disableIcon && <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />}
                    </Button>
                  </FormControl>
                </PopoverTrigger>
